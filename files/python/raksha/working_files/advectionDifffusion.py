...
# mesh = dfx.mesh.create_interval(MPI.COMM_WORLD, int(argv[1]), nx=int(argv[1]))
mesh = dfx.mesh.create_unit_interval(comm=MPI.COMM_WORLD, nx=int(argv[1]))
LEFT = 1; RIGHT = 2

# Tag Mesh facets for BCs
def left(x):   return np.isclose(x, 0.0)
def right(x):  return np.isclose(x, 1.0)
...
beta_value = 0.5 # Upwind parameter
...
ds = ufl.Measure('ds', domain=mesh) # Exterior facet integrals
dS = ufl.Measure('dS', domain=mesh) # Interior facet integrals

# Finite elements for the concentration
hf = 1/int(argv[1]) # Cell diameter, assumes constant mesh spacing
...
f = dfx.fem.Constant(mesh, dfx.default_scalar_type(0)) # Source term 
beta  = dfx.fem.Constant(mesh, dfx.default_scalar_type(beta_value)) # Upwind parameter
...
a1 = dot(grad(w), (D + (10*beta*hf))* grad(c) - u * c) * dx # Flux term integrated by parts


# a2 = beta / hf * c * w * ds(RIGHT)
