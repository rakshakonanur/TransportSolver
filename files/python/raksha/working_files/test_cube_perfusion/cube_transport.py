...
class TransportSolver:
    ...
    def setup(self):
        ...
        velocity_scale = 0.1
        u.interpolate(lambda x: (np.sin(x[0])*np.cos(x[1])*np.cos(x[2]),-np.cos(x[0])*np.sin(x[1])*np.cos(x[2]),np.zeros_like(x[0]))) # div(u)=0 by construction
        ...
        self.bc_func.interpolate(lambda x: np.abs(np.sin(np.pi*3*x[1])))
        ...
        # L += beta / hf * self.bc_func*w * self.ds(LEFT) # Weakly enforce Dirichlet BC using Nitsche's method
